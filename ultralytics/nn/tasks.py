...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in (Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, DSConv, Focus,
            ...
            c2 = ch[f]
            args = [ch[f], *args]
        elif m is HighFrequencyPrior:
            c2 = 1
            args = [ch[f], *args]
        elif m is HighFrequencyPriorGuidance:
            c2 = ch[f[1]]
        elif m is MSPConv:
            c1, c2 = ch[f], args[0]
            if c2 != nc:
            ...
            if c2 != nc:
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            args = [[ch[x] for x in f], c2, *args[1:]]
        elif m in (OrthogonalChannelAttention, HaarWaveletConv, CHFB, CSPMSEIE, MSASODAttention, MSAOriginalSODAttention, LGCDFSODAttention, GCRSODAttention, WFCSODAttention, StatisticalFeatureAttention, MDGA):
            c2 = ch[f]
            args = [ch[f], *args]
