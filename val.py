...
if __name__ == '__main__':
    model_path = "runs/train/RT-DETR-R18-MSAInnov-P2Decoder/weights/best.pt"
    model = RTDETR(model_path) # 选择训练好的权重路径
    result = model.val(data='./dataset/A_drowning_person.yaml',
                      split='val', # split可以选择train、val、test 根据自己的数据集情况来选择.
                      imgsz=640,
                      batch=4,
                      project='runs/val',
                      name='RT-DETR-R18-MSAInnov-P2Decoder',
                      )
